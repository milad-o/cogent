    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute(task, context):
            ...
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute("analyst", "Analyze data", context)
                await coordinator._subagent_registry.execute("researcher", "Research topic", context)
...
async def test_subagent_registry_cleared_before_run():
    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute_first(task, context):
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute("analyst", "First task", context)
            return "First result"
        ...
        assert second_response.metadata.tokens.total_tokens == 15  # Only coordinator tokens
        ...
        async def run(self, task, context=None):
            return Response(
                ...
                metadata=ResponseMetadata(
                    ...
                    tokens=TokenUsage(prompt_tokens=25, completion_tokens=75, total_tokens=100),
                    duration=2.5,
                ),
            )
...
async def test_no_delegation_chain_without_subagent_execution():
    ...
    class MockSubagent:
        ...
        async def run(self, task, context=None):
            return Response(content="Result", metadata=ResponseMetadata(agent=self.name, model="mock"))
