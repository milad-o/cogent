...
class TestCreateChatIntegration:
    ...
    def test_create_chat_single_arg(self):
        ...
        with patch(
            "cogent.models.openai.OpenAIChat.__init__", return_value=None
        ) as mock_init:
            try:
                create_chat("gpt4")
            except Exception:
...
class TestErrorHandling:
    ...
    def test_none_model_with_config(self):
        ...
        with patch(
            "cogent.config.get_provider_config", return_value={"default": "gpt4"}
        ), patch("cogent.models.openai.OpenAIChat.__init__", return_value=None):
            try:
