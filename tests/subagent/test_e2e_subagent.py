        ...
        async def mock_execute(task, context):
            ...
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute(
                    "data_analyst", "Analyze sales data showing 20% growth", context
                )
...
async def test_multiple_subagent_delegation():
    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute(task, context):
            ...
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute(
                    "analyst", "Analyze market trends", context
                )
                await coordinator._subagent_registry.execute(
                    "researcher", "Research competitors", context
                )
        ...
        mock_executor._last_messages = [
            AIMessage(
                ...
                metadata=MessageMetadata(
                    tokens=TokenUsage(
                        prompt_tokens=40, completion_tokens=80, total_tokens=120
                    )
                ),
            )
        ]
        ...
        with patch.object(analyst, "run", new_callable=AsyncMock) as mock_analyst:
            ...
            # Mock researcher
            with patch.object(
                researcher, "run", new_callable=AsyncMock
            ) as mock_researcher:
                ...
                response = await coordinator.run(
                    "Analyze market and research competitors"
                )
...
async def test_subagent_error_handling():
    ...
    class FailingSubagent:
        def __init__(self):
            ...
            self.config = type(
                "obj", (object,), {"description": "An agent that fails"}
            )()
    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute(task, context):
            try:
                if hasattr(coordinator, "_subagent_registry"):
                    await coordinator._subagent_registry.execute(
                        "failing_agent", "Do something", context
                    )
            except RuntimeError:
...
async def test_token_accuracy_with_real_delegation():
    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute(task, context):
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute(
                    "calculator", "Calculate 15 * 8", context
                )
...
async def test_context_preservation_e2e():
    ...
    class ContextAwareSubagent:
        def __init__(self):
            ...
            self.config = type(
                "obj", (object,), {"description": "Context aware agent"}
            )()
    ...
    with patch("cogent.executors.create_executor") as mock_executor_factory:
        ...
        async def mock_execute(task, context):
            if hasattr(coordinator, "_subagent_registry"):
                await coordinator._subagent_registry.execute(
                    "context_agent", "Check context", context
                )
        ...
        response = await coordinator.run(
            "Test context propagation", context=test_context
        )
