...
async def demo_knowledge_graph_retriever() -> None:
    ...
    docs = [
        Document(
            text="The Amazon rainforest is located in South America. It produces 20% of Earth's oxygen."
        ),
        Document(
            text="The Great Barrier Reef is in Australia. It contains over 400 species of coral."
        ),
        Document(
            text="Mount Everest is in the Himalayas. It is the tallest mountain on Earth."
        ),
    ]
...
async def demo_multi_representation_retriever() -> None:
    ...
    results = await retriever.retrieve(
        query, k=2, query_type=QueryType.AUTO, include_scores=True
    )
    for r in results:
