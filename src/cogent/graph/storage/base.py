...
class Storage(Protocol):
    ...
    def get_relationships(
        ...
        Example:
            >>> # Get all relationships for an entity
            >>> rels = backend.get_relationships("person:alice")
            >>> # Get all KNOWS relationships
            >>> knows = backend.get_relationships(relation="KNOWS")
            >>> # Get specific entity's KNOWS relationships
            >>> alice_knows = backend.get_relationships("person:alice", "KNOWS")
        """
        ...
    ...
    def query(self, pattern: str) -> list[dict[str, object]]:
        ...
        Example:
            >>> results = backend.query("? -KNOWS-> person:bob")
            >>> # Returns [{"source": Entity(...), "relation": "KNOWS", "target": Entity(...)}]
        """
        ...
    ...
    def find_path(
        ...
        Example:
            >>> path = backend.find_path("person:alice", "person:charlie")
