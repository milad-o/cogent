"""Flow-level checkpointing for crash recovery.

Provides persistent state management for multi-agent flows, enabling:
...
class FlowState:
    """Serializable snapshot of a flow's execution state.
...
class Checkpointer(Protocol):
    ...
    Implementations must be async-compatible for use in flows.
    
    Example:
        ```python
        checkpointer = MemoryCheckpointer()
        flow = Flow(checkpointer=checkpointer)
