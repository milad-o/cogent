...
The `RAG` capability wraps retrievers and provides document search tools to agents:

```python
from agenticflow import Agent
from agenticflow.capabilities import RAG
from agenticflow.capabilities.rag import RAGConfig, CitationStyle
from agenticflow.retriever import DenseRetriever
from agenticflow.vectorstore import VectorStore

# Simple usage - just a retriever
store = VectorStore(embeddings=embeddings)
await store.add_texts(["doc 1", "doc 2", "doc 3"])
...
rag = RAG(
    ...
    config=RAGConfig(
        ...
        citation_style=CitationStyle.NUMERIC,  # [1], [2], [3]
    ),
)

agent = Agent(model=model, capabilities=[rag])
response = await agent.run("What does doc 1 say?")
...
| `bibliography` | `False` | Enable metadata storage for bibliography |
| `bibliography_fields` | `()` | Metadata fields to show (e.g., `("author", "date")`) |
| `citation_style` | `NUMERIC` | Bibliography citation format |
| `max_chars` | `None` | Truncate passages to this length |

### Citation Styles

```python
from agenticflow.capabilities.rag import CitationStyle

# NUMERIC (default): [1] **source.pdf** p.5 — *author: Smith*
# FOOTNOTE:          ¹ **source.pdf** p.5 — *author: Smith*
# INLINE:            [source.pdf] p.5 — *author: Smith*
# AUTHOR_YEAR:       (Smith, 2024) **source.pdf** p.5
```
